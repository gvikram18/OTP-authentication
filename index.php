...
$success = "";
$error_message = "";
$conn = mysqli_connect("localhost","root","ivtree123","otp");
...
if(!empty($_POST["submit_otp"])) {
	$result = mysqli_query($conn,"SELECT * FROM otp_expiry WHERE otp='" . $_POST["otp"] . "' AND is_expired!=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 3 MINUTE)");
	$count  = mysqli_num_rows($result);
	if(!empty($count)) {
		$result = mysqli_query($conn,"UPDATE otp_expiry SET is_expired = 1 WHERE otp = '" . $_POST["otp"] . "'");
		date_default_timezone_set('Asia/Kolkata');
		$responded=date("Y-m-d H:i:s");
		$result = mysqli_query($conn,"UPDATE otp_expiry SET modified_at='$responded' WHERE otp = '" . $_POST["otp"] . "'");
		$success = 2;	
	} else {
		$success =1;
